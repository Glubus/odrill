/**
 * Odrill Optimized Icons Bundle
 * Replaces full Lucide library with only used icons (approx 2KB vs full lib).
 */
...
window.lucide = {
  createIcons: function() {
    document.querySelectorAll('[data-lucide]').forEach(elem => {
      const name = elem.getAttribute('data-lucide');
      if (icons[name]) {
        ...
        // Copy any existing style or classes
        if (elem.getAttribute('class')) {
            const existingClasses = elem.getAttribute('class');
            svg.setAttribute('class', `lucide lucide-${name} ${existingClasses}`);
        }
        if (elem.getAttribute('style')) {
            svg.setAttribute('style', elem.getAttribute('style'));
        }
        if (elem.getAttribute('id')) {
            svg.setAttribute('id', elem.getAttribute('id'));
        }

        svg.innerHTML = icons[name];
        elem.replaceWith(svg);
      } else {
        console.warn(`[Odrill] Icon not found: ${name}`);
      }
    });
  }
};
